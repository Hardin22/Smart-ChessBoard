...
def start_game(update_label_callback, update_eval_bar_callback, difficulty, chessboard):
...
def make_move(move, update_label_callback, update_eval_bar_callback, chessboard):
    try:
        ...
        print(f"Player evaluation: {eval_score}")  # Aggiungi questa riga
        ...
        chessboard.update_board(board.fen())
        eval_score = evaluate_position(board)
        print(f"Engine evaluation: {eval_score}")  # Aggiungi questa riga
        update_eval_bar_callback(eval_score)
        update_label_callback(eval_score)
    except ValueError:
...
if __name__ == "__main__":
    ...
    start_gui(lambda x: print(x), lambda x: print(f"Eval: {x}"), 1, ChessBoard())
